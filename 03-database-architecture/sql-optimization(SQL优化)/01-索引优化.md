...
// 利用复合索引进行优化查询
func (r *UserRepository) FindUsersByEmailAndAge(email string, age int) ([]User, error) {
    query := `
    ...
    rows, err := r.db.Query(query, email, age)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
    ...
    for rows.Next() {
        var user User
        err := rows.Scan(&user.ID, &user.Email, &user.Name, &user.Age, &user.CreatedAt)
        if err != nil {
            return nil, err
        }
