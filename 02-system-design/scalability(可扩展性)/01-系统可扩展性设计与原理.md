...
```java
// LRU缓存实现
LinkedHashMap<String, User> cache = new LinkedHashMap<String, User>(16, 0.75f, true) {
    @Override
...
public class NotificationPublisher {
    public void publishEmailEvent(EmailEvent event) {
        Topic topic = session.createTopic("email.events");
...
public class EmailSubscriber {
    public void subscribe() {
        Topic topic = session.createTopic("email.events");
        MessageConsumer consumer = session.createDurableSubscriber(topic, "email-subscriber");
        
        consumer.setMessageListener(new MessageListener() {
            public void onMessage(Message message) {
                EmailEvent event = (EmailEvent) ((ObjectMessage) message).getObject();
                sendEmail(event);
            }
        });
    }
