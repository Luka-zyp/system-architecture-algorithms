...
**数据库设计的重要性**：
- **数据完整性**：确保数据的准确性和一致性，避免数据冗余和不一致
- **数据一致性**：确保数据在不同表之间的一致性，维持业务规则
- **提高性能**：优化数据库结构，减少查询时间和系统资源消耗
- **可维护性**：便于系统的维护和升级，降低维护成本
- **可扩展性**：便于系统的扩展和功能增强，适应业务增长

**数据库设计的原则**：
- **数据完整性原则**：使用约束（主键、外键、唯一、非空等）确保数据准确性
- **数据一致性原则**：通过事务和锁机制维护数据在并发环境下的一致性
- **性能原则**：优化查询路径，使用适当的索引和数据结构
- **可维护性原则**：使用清晰的命名规范和文档，便于后续维护
- **可扩展性原则**：设计灵活的架构，支持未来的业务增长
...
**违反2NF的例子**：
```sql
-- 违反2NF：非主键字段部分依赖于主键的一部分
CREATE TABLE bad_order_items (
    order_id INT,               -- 主键的一部分
    product_id INT,             -- 主键的一部分
    order_date DATE,            -- 只依赖于 order_id
    product_name VARCHAR(100),  -- 只依赖于 product_id
    quantity INT,               -- 完全依赖于主键
