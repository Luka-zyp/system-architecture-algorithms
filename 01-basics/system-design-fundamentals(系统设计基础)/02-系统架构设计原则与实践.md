...
系统架构设计是构建大规模分布式系统的核心环节。本文档将探讨系统架构设计的基本原则、设计模式、实践方法，以及如何在实际项目中应用这些原则来构建可扩展、可维护、 高可用的系统。
...
class SystemComponent(ABC):
    ...
    def add_dependency(self, dependency: 'SystemComponent'):
        """添加依赖"""
...
class UserService:
    ...
    def create_user(self, user_data: dict) -> dict:
        ...
        if not self.password_hasher.is_strong_password(user_data.get('password', '')):
...
class PasswordHasher:
    ...
    def hash(self, password: str) -> str:
        """哈希密码"""
        salt = self.secrets.token_hex(16)
        hashed = self.hashlib.sha256((password + salt).encode()).hexdigest()
    ...
    def verify(self, password: str, hashed_password: str) -> bool:
        ...
        try:
            ...
            computed_hash = self.hashlib.sha256((password + salt).encode()).hexdigest()
...
class NotificationService:
    ...
    def broadcast_notification(self, channel_type: str, message: str, recipients: List[str]) -> Dict[str, bool]:
        """广播通知"""
        results = {}
        for recipient in recipients:
            try:
                success = self.send_notification(channel_type, message, recipient)
...
class PresentationLayer(Layer):
    ...
    def add_formatter(self, formatter: Callable):
        """添加响应格式化器"""
        self.response_formatters.append(formatter)
    ...
    def process(self, request: Any) -> Any:
        ...
        # 转换请求格式
        processed_request = self._format_request(request)
        
        return processed_request
    
    def _format_request(self, request: Any) -> Any:
        """格式化请求"""
        # 简化实现
    ...
    def format_response(self, response: Any) -> Any:
        ...
        for formatter in self.response_formatters:
            formatted_response = formatter(formatted_response)
...
class DataAccessLayer(Layer):
    ...
    def register_repository(self, repo_name: str, repository: Any):
        """注册数据仓储"""
    ...
    def process(self, request: Any) -> Any:
        """处理数据操作"""
        if isinstance(request, dict) and 'data_action' in request:
            action = request['data_action']
            repository_name = request.get('repository', 'default')
            
            repository = self.repositories.get(repository_name)
            if repository and hasattr(repository, action):
...
class LayeredArchitecture:
    ...
    def __init__(self):
        self.presentation_layer = PresentationLayer("Presentation")
        self.business_layer = BusinessLogicLayer("BusinessLogic")
        self.data_layer = DataAccessLayer("DataAccess")
        
        # 建立层间连接
        self.presentation_layer.set_next_layer(self.business_layer)
        self.business_layer.set_next_layer(self.data_layer)
    
    def handle_request(self, request: Any) -> Any:
        """处理请求"""
        return self.presentation_layer.handle_request(request)
    
    def add_presentation_validator(self, validator: Callable):
        """添加表现层验证器"""
        self.presentation_layer.add_validator(validator)
    
    def add_business_rule(self, rule: Callable):
        """添加业务规则"""
        self.business_layer.add_business_rule(rule)
    
    def register_repository(self, repo_name: str, repository: Any):
        """注册数据仓储"""
        self.data_layer.register_repository(repo_name, repository)
    
    def get_architecture_status(self) -> Dict:
        """获取架构状态"""
        return {
            'presentation_layer': self.presentation_layer.get_status(),
            'business_layer': self.business_layer.get_status(),
            'data_layer': self.data_layer.get_status()
        }
```
